...
VDMS implements a the typical client-server architecture that handles
client queries transactionally and concurrently, as most of the most common
relational and non-relational database systems
\cite{mysql, postgresql, chang2008bigtable}.
...
VDMS provides a property \textit{graph model} abstraction with the traditional
...
VDMS API provides mechanism to insert and connect Images, Videos, BoundingBoxes,
Frames, and Descriptors (feature vectors), together with any metadata
associated with the objects.
Each object is a node in the graph.
The metadata associated to each object is is modeled as
"properties" of the node in the graph.
Users can query, filter, and retrieve this objects based
on its metadata (the "properties" associated with them).
...
and over the abstractions the relational model support (tables, columns).
A SQL query can then retrieve the \textit{computed average} "salary" of all
...
of multiple systems and APIs to access different portions of the data they need
...
The Graph Engine used for in VDMS is the
...
atomicity, consistency, isolation, and durability
(ACID) properties expected from databases.
PMGD comes as a C++ library that VDMS uses for its internal query engine.
...
It enables simple visual data handling and processing
(i.e., basic building block operations like crop, resize, etc).
