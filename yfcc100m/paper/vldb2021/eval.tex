...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/comparison_system}
...
of the image), and geo-location information (latitude and longitude)
...
for \textit{q2} described above (\textit{1tag\_geo\_resize}),
...
computation are performed on them (i.e., a resize plus
normalization done on images before passing them through a neural network).
...
When the resize operation is performed, both baselines do very similar
...
operations happen on the client side (or wherever the analytics computation happens),
...
re-purposing systems that were designed to handle types of data other than visual.
