...
logs_dir = "timelogs_s4_p2"
# files = fnmatch.filter(os.listdir(logs_dir), '*.log')
# files.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))

files_array = [1,2,4,8,16,32,48,56,64,72,96,112]
...
# plt.xticks(files_array)

xticks = list(range(len(files_array)))
# xticks = str(files_array)
plt.xticks(xticks, files_array)
