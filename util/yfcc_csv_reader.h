...
void map2Location(Jarvis::Node &nmedia , Jarvis::Graph& db)
{
    ...
    for (Jarvis::NodeIterator i = db.get_nodes("place", pp2); i; i.next())
    {
...
int insertNewMedia(std::string file, Jarvis::Graph& db)
{
    ...
    db.create_index(Jarvis::Graph::NodeIndex, "media", "id", Jarvis::PropertyType::Integer  );
    txi.commit();
    ...
    {     // '\n' is the default delimiter
        ...
        Jarvis::NodeIterator i = db.get_nodes("media", pps1);

        if (!i)
        {
            Jarvis::Node &nmedia = db.add_node("media");
            ...
            {     // '\n' is the default delimiter
                ...
                while(std::getline(iss, token, ','))
                {
                    ...
                    Jarvis::NodeIterator i = db.get_nodes("label", pps1);
    ...
    std::cout << "Average Transaction Time: " << chrono.getElapsedStats().averageTime_ms << std::endl;

    std::cout << "Elements added: " << counter - counterRep << std::endl;
    std::cout << "Elements Repeted: " << counterRep << std::endl;

    Jarvis::Transaction tx(db, Jarvis::Transaction::ReadWrite);
    for (Jarvis::NodeIterator i = db.get_nodes("media"); i; i.next()) // should be only 0 or 1 though
    {
        mediaCounter++;
    }
    tx.commit();
